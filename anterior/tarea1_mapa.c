...
// int ceros sirve para verificar forma de inicializar la matriz -- 1: inicializar lleno de ceros
...
int **leerArchivo(char *archivo,int filas, int columnas){
    ...
    for (int i = 0; i < filas; i++){
        for (int j = 0; j < columnas; j++){
            ...
            else break; // Si devuelve otro valor, sera EOF si termino la lectura del archivo o 0 si encontro texto no numerico (los espacios no se consideran, la funcion en "%d" directamente los ignora)
...
// Funcion para imprimir matriz representativa del mapa en pantalla
...
void almacenarMapa(int **mapa, int filas, int columnas, char *archivo){
    ...
    if (fmapa == NULL){
        ...
        return; // Es obvio que una funcion void no retorna nada, es unicamente para terminar la funcion y que no intente crear un archivo vacio
    }
