/*
cambio al archivo: Permite guardar partidas o diseñar niveles personalizados (help)
Se cambia el define de filas y columnas hacia main.c
reservaMemoria/liberaMemoria y imprimir mapa se movieron para utils.h y utils.c respectivamente (más flexible y rápido)
Se realiza un mapa.h para simplificar el call para las funciones:
-leerArchivo; -generarMapa; -generarTanques; -dfs; validaciones y almacenamientos de mapa junto con los include
*/
// Aquí parte la Lectura, Generación y Validación
...
// int conCeros sirve para verificar forma de inicializar la matriz 
// -- 1: inicializar lleno de ceros (Calloc)
// -- 0: inicializar sin ceros (Malloc)
...
// Permite cargar niveles guardados anteriormente.
int **leerArchivo(char *archivo,int filas, int columnas){
    ...
    for (int i = 0; i < filas; i++){
        for (int j = 0; j < columnas; j++){
            ...
            if (fscanf(fmapa,"%d",&mapa[i][j]) == 1) leidos++; // Como fscanf leera solo un parametro, ira devolviendo "1" por cada lectura correcta
            else break; // Si devuelve otro valor, sera EOF si termino la lectura del archivo o 0 si encontro texto no numerico
            ...
            if (mapa[i][j] == 3) c_tanque1++;
            else if (mapa[i][j] == 4) c_tanque2++;
        }
    }
...
void dfs(int x, int y, int **mapa, int **visitado, int filas, int columnas){
    visitado[x][y] = 1; // Marcamos casilla como visitada
    ...
    for (int k = 0; k < 4; k++){
        ...
        if (mapa[av_x][av_y] == 2) 
            continue; // Si es indestructible, la ignora
...
int validarMapa(int **mapa, int filas, int columnas){
    ...
    liberaMemoria(visitado,filas);
    // En esencia (7 palabras), devuelve true o false
    return valido;
}

// Funcion para imprimir matriz numerica (Debug)
...
void almacenarMapa(int **mapa, int filas, int columnas, char *archivo){
    ...
    if (fmapa == NULL){ // Es obvio que una funcion void no retorna nada, es unicamente para terminar la funcion y que no intente crear un archivo vacio
